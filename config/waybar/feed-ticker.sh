...
# Build API URL - get recent headlines from last 48 hours
HOURS_AGO_48=$(( $(date +%s) - (48 * 60 * 60) ))
FEED_URL="https://feed.internetuniverse.org/p/api/greader.php/reader/api/0/stream/contents/reading-list?n=50&ot=${HOURS_AGO_48}&output=json"
...
# Use time-based rotation (rotates every 5 seconds based on current epoch)
index=$(( ($(date +%s) / 5) % 50 ))

# Get headline at current index
