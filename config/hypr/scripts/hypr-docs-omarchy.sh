#!/bin/bash
# Omarchy-native Hyprland Documentation Browser
...
EOF
    ...
    case "$action" in
        *"Neovim"*)
            ...
            $TERMINAL --class=hypr-docs-editor --title="📚 Hyprland Docs" \
                -e bash -c "${EDITOR:-nvim} +${line:-1} \"$full_path\"" &

            # Tag after spawn
            sleep 0.15
            hyprctl dispatch tagwindow +omarchy-editor class:^hypr-docs-editor$ >/dev/null 2>&1
            ;;
        ...
        *"Preview"*)
            ...
            if command -v bat &> /dev/null; then
                $TERMINAL --class=hypr-docs-preview --title="📚 ${file##*/}" \
            ...
            # Tag after spawn
            sleep 0.15
            hyprctl dispatch tagwindow +omarchy-docs class:^hypr-docs-preview$ >/dev/null 2>&1
            ;;
...
main() {
    ...
    query=$(omarchy-launch-hyprland-docs)
    ...
    if [[ -z "$results" ]]; then
        notify-send "📚 Hyprland Docs" "No results found for: $query" -u normal -t 3000
        exit 0
    fi
